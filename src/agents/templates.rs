...
///
/// Declared as a subagent (not a teammate) because its task is bounded and
/// its results are summarized back — it doesn't need to initiate discussion
/// with other reviewers. However, the team lead's orchestration prompt tells
/// teammates to spawn this subagent and then share notable findings via the
/// team message system if cross-domain coordination is needed.
...
///
/// Runs as a background subagent because it is I/O-heavy (cargo audit,
/// network checks) and its result only needs to be summarized back to the
/// lead — it does not need to communicate with other teammates in real time.
...
/// Subagent: depth-first security reviewer with git worktree isolation.
///
/// Spawned once per review task during Phase 3 (Deep Review). Each reviewer
/// gets a randomly-selected starting file (weighted by scout score) and
/// performs a depth-first security audit, recursively following call chains.
/// Works in an isolated git worktree so it can instrument code and write PoCs.
pub const REVIEWER: &str = r#"---
name: reviewer
description: >
